...
namespace DiceDemo.M1
{
    public sealed class BattleFlow : MonoBehaviour
    {
        ...
        private Texture2D _aimRingTexture, _aimButtonTexture;
        ...
        private Vector2 _aimCenterGui, _aimKnobGui;
        private Vector3 _aimStartBoardPoint, _aimDirection = Vector3.forward;
        ...
        private void Update()
        {
            ...
            if (!_isAiming)
            {
                ...
                {
                    _isAiming = true;
                    _aimKnobGui = _aimCenterGui;
                    if (!TryGetBoardPoint(new Vector3(_aimCenterGui.x, Screen.height - _aimCenterGui.y, 0f), out _aimStartBoardPoint))
                        _aimStartBoardPoint = new Vector3(BattleBalance.LaunchX, BattleBalance.BoardSurfaceY, BattleBalance.LaunchZ);
            ...
            if (Input.GetMouseButtonUp(0))
            {
                UpdateAimFromGui(mouseGui);
                _isAiming = false;
                if (_aimStrength >= AimStartThreshold / AimRadius)
                    LaunchToward(_aimDirection, _aimStrength);
                _aimKnobGui = _aimCenterGui;
            }
        }

        private void UpdateAimFromGui(Vector2 mouseGui)
        {
            Vector2 offset = mouseGui - _aimCenterGui;
            if (offset.sqrMagnitude > AimRadius * AimRadius) offset = offset.normalized * AimRadius;
            _aimKnobGui = _aimCenterGui + offset;
            _aimStrength = Mathf.Clamp01(offset.magnitude / AimRadius);

            Vector3 boardPoint;
            if (!TryGetBoardPoint(new Vector3(_aimKnobGui.x, Screen.height - _aimKnobGui.y, 0f), out boardPoint)) return;
            Vector3 dragDelta = boardPoint - _aimStartBoardPoint;
            dragDelta.y = 0f;
            if (dragDelta.sqrMagnitude > 0.0001f) _aimDirection = -dragDelta.normalized;
        }
        ...
        private void LaunchToward(Vector3 direction, float strength)
        {
            ...
            Vector3 target; Vector3 velocity; float flightTime;
            if (!AimTrajectoryMath.TryCalculateLaunch(origin, horizontalDirection, strength, Physics.gravity, out target, out velocity, out flightTime)) return;
            ...
            // BattleDie uses ForceMode.Impulse; convert the desired velocity to impulse
            // so the launch distance is independent of the template's mass.
            die.Launch(velocity * die.Body.mass, new Vector3(RandomSigned(7f), RandomSigned(7f), RandomSigned(7f)));
            AddLog("已投掷：" + KindShort(_nextKind) + "，第" + _nextLevel + "级，力度" + Mathf.RoundToInt(strength * 100f) + "%"); RollNextDie(); _phase.TryTransition(BattlePhase.PhysicsResolution); _physicsStartedAt = Time.time; _allSleepingSince = -1f; _physicsTimedOut = false; _roundRoutine = StartCoroutine(ResolveRound());
        }
        ...
        private void BeginRound() { _round++; _player.ClearShield(); AddLog("第" + _round + "回合：拖动圆盘中心按钮并松开投掷"); StartCoroutine(BeginAimAfterDelay()); }
        ...
        private static Texture2D MakeCircleTexture(int size, Color fill, Color edge, int edgeWidth)
        {
            Texture2D texture = new Texture2D(size, size, TextureFormat.RGBA32, false) { name = "AimCircle" };
            float center = (size - 1) * 0.5f, outer = center, inner = Mathf.Max(0f, outer - edgeWidth);
            Color[] pixels = new Color[size * size];
            for (int y = 0; y < size; y++) for (int x = 0; x < size; x++)
            {
                float distance = Vector2.Distance(new Vector2(x, y), new Vector2(center, center));
                pixels[y * size + x] = distance <= inner ? fill : distance <= outer ? edge : Color.clear;
            }
            texture.SetPixels(pixels); texture.Apply(); return texture;
        }

        private void UpdateAimCenter()
        {
            Vector3 screen = _camera.WorldToScreenPoint(new Vector3(BattleBalance.LaunchX, BattleBalance.LaunchY, BattleBalance.LaunchZ));
            _aimCenterGui = new Vector2(screen.x, Screen.height - screen.y);
            if (!_isAiming) _aimKnobGui = _aimCenterGui;
        }
        ...
        private void DrawAimGuide()
        {
            ...
            UpdateAimCenter();
            if (_aimRingTexture == null) _aimRingTexture = MakeCircleTexture(192, new Color(0.05f, 0.20f, 0.32f, 0.30f), new Color(0.35f, 0.85f, 1f, 0.95f), 5);
            if (_aimButtonTexture == null) _aimButtonTexture = MakeCircleTexture(56, new Color(1f, 0.55f, 0.12f, 1f), new Color(1f, 0.92f, 0.55f, 1f), 3);
            GUI.DrawTexture(new Rect(_aimCenterGui.x - AimRadius, _aimCenterGui.y - AimRadius, AimRadius * 2f, AimRadius * 2f), _aimRingTexture);

            Vector2 knob = _isAiming ? _aimKnobGui : _aimCenterGui;
            GUI.DrawTexture(new Rect(knob.x - AimKnobRadius, knob.y - AimKnobRadius, AimKnobRadius * 2f, AimKnobRadius * 2f), _aimButtonTexture);
            GUI.Label(new Rect(_aimCenterGui.x - 95f, _aimCenterGui.y + AimRadius + 10f, 190f, 24f), _isAiming ? "拖动瞄准，松开投掷" : "拖动中心按钮投掷", _smallStyle);

            if (!_isAiming || _aimStrength <= 0.001f) return;
            Vector3 origin = new Vector3(BattleBalance.LaunchX, BattleBalance.LaunchY, BattleBalance.LaunchZ), target, velocity;
            float flightTime;
            if (!AimTrajectoryMath.TryCalculateLaunch(origin, _aimDirection, _aimStrength, Physics.gravity, out target, out velocity, out flightTime)) return;
            Vector2 previous = WorldToGui(origin);
            for (int i = 1; i <= AimTrajectorySegments; i++)
            {
                float t = flightTime * i / AimTrajectorySegments;
                Vector3 point = origin + velocity * t + 0.5f * Physics.gravity * t * t;
                if (point.y < BattleBalance.BoardSurfaceY) point.y = BattleBalance.BoardSurfaceY;
                Vector2 current = WorldToGui(point);
                DrawLine(previous, current, new Color(1f, 0.83f, 0.22f, 0.92f), 4f);
                previous = current;
            }
            Vector2 targetGui = WorldToGui(target);
            GUI.color = new Color(1f, 0.35f, 0.12f, 0.9f);
            GUI.DrawTexture(new Rect(targetGui.x - 7f, targetGui.y - 7f, 14f, 14f), Texture2D.whiteTexture);
            GUI.color = Color.white;
            GUI.Label(new Rect(_aimCenterGui.x - 70f, _aimCenterGui.y - AimRadius - 28f, 140f, 24f), "力度 " + Mathf.RoundToInt(_aimStrength * 100f) + "%", _smallStyle);
        }
